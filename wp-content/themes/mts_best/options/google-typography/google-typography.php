...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', 'best' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'best' ).'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'best' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'best' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'best' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'best' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'best' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'best' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'best' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'best' ).'</li><li><b>'.__('Font Variant', 'best' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'best' ).'</li><li><b>'.__('Font Size', 'best' ).'</b> - '.__('The size you would like this font to be.', 'best' ).'</li><li><b>'.__('Font Color', 'best' ).'</b> - '.__('The color you\'d like to use for this font.', 'best' ).'</li><li><b>'.__('CSS Selectors', 'best' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'best' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'best' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'best' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'best' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'best' ).':</b> '.__('#selector', 'best' ).'</li><li><b>'.__('Classes:', 'best' ).':</b> '.__('.selector', 'best' ).'</li><li><b>'.__('HTML Tags', 'best' ).':</b> '.__('span', 'best' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'best' ).':</b> '.__('#selector span.date', 'best' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'best' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', 'best' );
		$loading							= __('Loading Your Collections', 'best' );
		$add_new        			= __('Add New Collection', 'best' );
		$reset        				= __('Reset Collections', 'best' );
		$preview_text   			= __('Type in some text to preview...', 'best' );
		$collection_title 			= __('New Collection', 'best' );
        //$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'best' );
		//$font_family_title  	= __('Font family...', 'best' );
        $std_font_family_title  	= __('Standard Fonts', 'best' );
        $google_font_family_title  	= __('Google Fonts', 'best' );
		$font_variant_title 	= __('Variant...', 'best' );
		$font_size_title 			= __('Size...', 'best' );
        $backup_font_title          = __('Fallback:', 'best' );
		$css_selectors_title	= __('CSS Selectors:', 'best' );
        $additional_css_title	= __('Additional CSS:', 'best' );
		$delete_button_text   = __('Delete', 'best' );
		$save_button_text			= __('Save', 'best' );
		
		$welcome_title 				= __('Theme Typography', 'best' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'best' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'best' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'best' );
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', 'best' );
		$step_2_title 				= __('2. Customize It', 'best' );
		$step_2_desc					= __('Pick a size, variant, color and more.', 'best' );
		$step_3_title 				= __('3. Attach It', 'best' );
		$step_3_desc					= __('Attach your font to any CSS selector(s).', 'best' );
		$year									= date("Y");
        $themename = MTS_THEME_NAME;
        
        $character_sets_title = __('Character sets', 'best' );
        $character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'best' );
        ...
        $character_sets = array(
            'latin' => __('Latin', 'best' ),
            'latin-ext' => __('Latin Extended', 'best' ),
            'cyrillic' => __('Cyrillic', 'best' ),
            'cyrillic-ext' => __('Cyrillic Extended', 'best' ),
            'greek' => __('Greek', 'best' ),
            'greek-ext' => __('Greek Extended', 'best' ),
            'vietnamese' => __('Vietnamese', 'best' ),
            'khmer' => __('Khmer', 'best' ),
            'devanagari' => __('Devanagari', 'best' ),
        );
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
	            'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'best' ),
	            'delete_confirm' => __("Are you sure you want to delete this collection?", 'best' )
	        ));
